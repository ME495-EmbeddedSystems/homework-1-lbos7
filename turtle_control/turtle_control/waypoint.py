...
class Waypoint(Node):
    ...
    def __init__(self):
        ...
        self.teleport_abs_client = self.create_client(TeleportAbsolute, 'teleport_absolute')
        self.teleport_rel_client = self.create_client(TeleportRelative, 'teleport_relative')
    ...
    # def load_callback(self, request, response):
    #     self.reset_client.call_async(Empty.Request(name='turtle1'))
    #     is_first_point = True
    #     first_point = {}
    #     prev_point = {}
    ...
    #                                       [prev_point.x, prev_point.y])
    #                 self.teleport_abs_client.call_async(TeleportAbsolute.Request(
    #                     x=request.waypoints[i].x, y=request.waypoints[i].y, theta=0))
    #                 prev_point = request.waypoints[i]
    #             else:
    #                 self.teleport_abs_client.call_async(TeleportAbsolute.Request(
    #                     x=request.waypoints[i].x, y=request.waypoints[i].y, theta=0))
    ...
    async def draw_x(self, points):
        for point in points:
            await self.path_client.call_async(SetPen.Request(r=255, g=0, b=0,
                                                        width=3, off=1))
            self.teleport_abs_client.call_async(TeleportAbsolute.Request(
